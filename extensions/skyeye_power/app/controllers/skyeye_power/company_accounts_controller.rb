...
#expose(:resources) {:company_accounts}
      ...
      def create
        ...
        respond_with(@account) do |format|
          if @account.save
            format.html { redirect_to [company,resources], notice: '账户新建成功' }
          else
            format.html { redirect_to [company,resources], alert:"账户新建失败:#{@account.errors.full_messages.join(',')}" }
          end
        end    
      end
      ...
      def update
        ...
        respond_with(@account) do |format|
          if @account.update_attributes(params[resources.to_s.singularize])
            format.html { redirect_to [company,resources], notice: '账户修改成功' }
          else
            format.html { redirect_to [company,resources], alert: "账户修改失败:#{@account.errors.full_messages.join(',')}" }
          end
        end
      end
